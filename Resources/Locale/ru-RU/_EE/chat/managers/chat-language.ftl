chat-language-Ilasir-name = Иласир
chat-language-Kelael-name = Кела'эл
chat-language-Universal-name = Универсальный
chat-language-Psychomantic-name = Психомантический
chat-language-Bubblish-name = Пузырчатый
chat-language-RootSpeak-name = Язык корней
chat-language-Nekomimetic-name = Некомиметический
chat-language-Draconic-name = Синта'Унати
chat-language-Azaziba-name = Синта'Азазиба
chat-language-SolCommon-name = Сол Общий
chat-language-NovuNederic-name = Нову-Недерик
chat-language-NovaCygniBasic-name = Нова-Цигни Бейсик
chat-language-TauCetiBasic-name = Бейсик
chat-language-Tradeband-name = Торговый
chat-language-Freespeak-name = Фриспик
chat-language-Elyran-name = Элиран
chat-language-Canilunzt-name = Канилунцт
chat-language-Moffic-name = Моффик
chat-language-RobotTalk-name = Бинарный
chat-language-Calcic-name = Кальцик
chat-language-ValyrianStandard-name = Валирийский
chat-language-Sign-name = Жестовый
chat-language-Marish-name = Мариш
chat-language-Chittin-name = Хитин
chat-language-Xeeplian-name = Зиплиан
chat-language-Hydraspeak-name = Хи'драв'тха

# Floofstation:
chat-language-NewKinPidgin-name = Ка'ракк

# Дополнительные языки
chat-language-NalRasan-name = Нал'расан
chat-language-SiikTajr-name = Сиик'таджр
chat-language-SiikMaas-name = Сиик'маас
chat-language-YaSsa-name = Я'сса
chat-language-Delvahii-name = Дельвахии
chat-language-Shadowmind-name = Тенеразум
chat-language-Qiilour-name = Квилур

# Языки животных
chat-language-Cat-name = Кошачий
chat-language-Chicken-name = Куриный
chat-language-Cow-name = Коровий
chat-language-Crab-name = Крабий
chat-language-Deer-name = Олений
chat-language-Dog-name = Собачий
chat-language-Duck-name = Утиный
chat-language-Eldritch-name = Древний
chat-language-Fox-name = Лисий
chat-language-Hissing-name = Шипящий
chat-language-Kangaroo-name = Кенгуру
chat-language-Kobold-name = Кобольдий
chat-language-Monkey-name = Обезьяний
chat-language-Mouse-name = Мышиный
chat-language-Penguin-name = Пингвиний
chat-language-Pig-name = Свиной
chat-language-Sheep-name = Овечий
chat-language-Xeno-name = Ксеноморфный
chat-language-DroneTalk-name = Дронов

# Mono
chat-language-Blob-name = Блобовый