## Components

stamp-component-signee-name = { $user }
stamp-component-stamped-name-psychologist = Психолог
stamp-component-stamped-name-stc = Диспетчер Аванпоста
stamp-component-stamped-name-sr = Комендант
stamp-component-stamped-name-doctor = Доктор
stamp-component-stamped-name-engineer = Инженер
stamp-component-stamped-name-bailiff = Капитан ТСФ
stamp-component-stamped-name-confidential = Комендант
stamp-component-stamped-name-doc = Директор медицинского центра
stamp-component-stamped-name-pal = Представитель Юр. Департамента
stamp-component-stamped-name-plant-manager = Директор Электростанции
