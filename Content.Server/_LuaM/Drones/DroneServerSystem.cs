...
namespace Content.Server._LuaM.Drones.Systems;
...
public sealed class DroneSystem : EntitySystem
{
    ...
    public void StartControlDrone(EntityUid drone, EntityUid host, EntityUid controller)
    {
        ...
        if (!_powerCell.TryGetBatteryFromSlot(drone, out var batteryUid, out var battery) ||
            battery.CurrentCharge <= droneComp.Wattage)
        {
            _popupSystem.PopupEntity(Loc.GetString("drone-no-power"), host, host, PopupType.MediumCaution);
            return;
        }

        var chargeLevel = GetChargeLevel(battery);
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var host))
        {
            ...
            if (!_powerCell.TryGetBatteryFromSlot(host.Drone.Value, out var batteryUid, out var battery))
            {
                _controller.DisconectDrone(controller, "drone-disconnect-no-power");
                continue;
            }

            if (!_battery.TryUseCharge(batteryUid.Value, drone.Wattage * frameTime, battery))
            {
                _controller.DisconectDrone(controller, "drone-disconnect-no-power");
            }
    ...
    private static float GetChargeLevel(BatteryComponent battery)
    {
        return battery.MaxCharge <= 0f
            ? 0f
