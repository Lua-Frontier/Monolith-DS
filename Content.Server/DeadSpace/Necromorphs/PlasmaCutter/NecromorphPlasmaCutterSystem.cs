...
public sealed class NecromorphPlasmaCutterSystem : EntitySystem
{
    ...
    private void OnHit(Entity<NecromorphPlasmaCutterComponent> ent, ref HitscanDamageDealtEvent args)
    {
        ...
        if (wounds.RemovedLegs < 2 && TryDetachFirstPart(target, BodyPartType.Leg, Vector2.Zero, ent.Comp.DetachedPartImpulse))
        {
        ...
        if (!TryDetachFirstPart(target, BodyPartType.Head, Vector2.Zero, ent.Comp.DetachedPartImpulse))
            return;
