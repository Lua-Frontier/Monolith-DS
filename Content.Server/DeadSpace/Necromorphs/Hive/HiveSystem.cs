...
public sealed class HiveSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var hivec, out var charges))
        {
            if (_timing.CurTime > hivec.NextTick)
            {
                ...
                _popup.PopupEntity(Loc.GetString($"Способность восстановилась, количество = {charges.Charges}"), uid, uid);
            }
        }
    }
    ...
    private void OnRaiseArmy(EntityUid uid, HiveComponent component, RatKingRaiseArmyActionEvent args)
    {
        ...
        if (charges != null)
            _charges.TryUseCharge(new Entity<LimitedChargesComponent?>(uid, charges));
    }
}
